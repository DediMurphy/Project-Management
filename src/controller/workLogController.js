...
export const getAllWorkLog = async (req, res, next) => {
  try {
    const dateParam = req.query.date; // ✅ perbaikan: gunakan dateParam

    console.log("🟢 Controller received date param:", dateParam);

    const worklogs = dateParam
      ? await workLogService.getWorkByDate(dateParam)
      : await workLogService.getAllWorkLog();
